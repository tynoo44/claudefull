...
export const useConversationPagination = () => {
  ...
  const loadConversationPage = useCallback(async (page: number, existingIds = new Set<string>()) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const checkAndLoadMore = useCallback((visibleIndex: number) => {
  ...
  const applyFilters = useCallback((
    ...
    if (searchTerm) {
      filtered = filtered.filter(conv => 
        (conv.leads.full_name || conv.leads.username)?.toLowerCase().includes(searchTerm.toLowerCase()) ||
