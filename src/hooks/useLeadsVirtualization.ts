...
export const useLeadsVirtualization = () => {
  ...
  const applyFiltersInternal = useCallback((leads: LeadWithMetadata[], filters: FilterOptions) => {
    let filtered = [...leads];
    const { searchTerm, selectedTags, statusFilter, procedenceFilter, sortBy, sortAscending } = filters;
        ...
        const newLeads = prev.map(lead => 
          lead.id === leadId 
            ? { 
                ...
                conversation_count: updatedLead.conversation_count || lead.conversation_count || 0,
