...
export const useGlobalCache = () => {
  ...
  const updateCache = useCallback((updates: Partial<CacheState>) => {
  ...
  const loadDashboardStats = useCallback(async (force = false) => {
    ...
    try {
      ...
      const conversationsByStatus = statusData?.reduce((acc, lead) => {
  ...
  const invalidate = useCallback((dataType: keyof CacheState['lastUpdated']) => {
    updateCache({
