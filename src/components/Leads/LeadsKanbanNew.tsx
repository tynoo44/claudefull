...
import { 
  Users, Target, Calendar, CheckCircle, AlertTriangle, 
  Handshake, DollarSign, Clock, ArrowUp, Snowflake, X,
  Maximize2, Minimize2
} from 'lucide-react';
...
export const LeadsKanbanNew: React.FC<LeadsKanbanNewProps> = ({
  darkMode,
  leads,
  onLeadUpdate
}) => {
  ...
  const handleLeadModalUpdate = useCallback((updatedLead: Lead) => {
  ...
  const totalWidth = useMemo(() => {
    ...
    return (normalColumns * columnWidth) + (minimizedCount * MINIMIZED_COLUMN_WIDTH) + (KANBAN_COLUMNS.length * 16); // 16px gap
