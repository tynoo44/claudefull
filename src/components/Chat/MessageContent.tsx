...
export const MessageContent: React.FC<MessageContentProps> = ({ content, className = '' }) => {
  ...
  const formatContent = (text: string): string => {
    ...
    formatted = formatted.replace(
      /\*\*([^*]+)\*\*:/g,
      '<div class="font-bold text-base mt-6 mb-3 text-purple-900 dark:text-purple-200 border-b border-purple-200 dark:border-purple-700 pb-2">$1:</div>',
    );
    ...
    formatted = formatted.replace(
      /^### (.*?)$/gm,
      '<h3 class="text-lg font-bold mt-6 mb-3 text-gray-900 dark:text-gray-100">$1</h3>',
    );
    formatted = formatted.replace(
      /^## (.*?)$/gm,
      '<h2 class="text-xl font-bold mt-6 mb-3 text-gray-900 dark:text-gray-100">$1</h2>',
    );
    formatted = formatted.replace(
      /^# (.*?)$/gm,
      '<h1 class="text-2xl font-bold mt-6 mb-3 text-gray-900 dark:text-gray-100">$1</h1>',
    );
    ...
    formatted = formatted.replace(
      /(<li class="ml-[46] list-(?:disc|decimal)">.*?<\/li>(?:<br \/>)?)+/g,
      match => {
