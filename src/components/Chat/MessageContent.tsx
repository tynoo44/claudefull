...
export const MessageContent: React.FC<MessageContentProps> = ({ content, className = '' }) => {
  ...
  const formatContent = (text: string): string => {
    ...
    formatted = formatted.replace(/📝 \*\*Opción (\d+).*?\*\*:(.*?)```([\s\S]*?)```/g, (match, num, desc, message) => {
      ...
      return `
      <div class="my-4 border rounded-lg overflow-hidden bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border-purple-200 dark:border-purple-700">
        <div class="px-4 py-2 bg-purple-100 dark:bg-purple-800/30 border-b border-purple-200 dark:border-purple-700">
          <div class="flex items-center gap-2">
            <span class="text-lg">📝</span>
            <strong class="text-purple-900 dark:text-purple-200">Opción ${num}</strong>
        ...
        <div class="p-4">
          <div class="bg-white dark:bg-gray-800 rounded-lg p-4 font-mono text-sm border border-purple-100 dark:border-gray-700 shadow-sm">
            <div class="select-all cursor-text hover:bg-purple-50 dark:hover:bg-purple-900/10 -m-4 p-4 rounded-lg transition-colors">
              ${messageClean}
            </div>
          </div>
          <div class="mt-2 text-xs text-purple-600 dark:text-purple-400 text-center">Click para seleccionar • Copia y pega en tu chat</div>
        </div>
      </div>`;
    });
    ...
    formatted = formatted.replace(/\*\*([^*]+)\*\*:/g, '<div class="font-bold text-base mt-4 mb-2 text-purple-900 dark:text-purple-200">$1:</div>');
    ...
    formatted = formatted.replace(/```([\s\S]*?)```/g, (match, code) => {
      ...
      return `<div class="bg-gray-900 text-gray-100 rounded-lg p-4 my-3 overflow-x-auto"><pre class="text-sm"><code>${code.trim()}</code></pre></div>`;
    });
