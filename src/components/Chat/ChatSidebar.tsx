...
import { Search, ChevronLeft, ChevronRight, MessageSquare, Filter, SortAsc, Hash, Calendar } from 'lucide-react';
...
export const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  const filteredAndSortedChats = chats
    .filter(chat => {
      const matchesSearch = chat.leadName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          chat.lastMessage.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesStatus = !statusFilter || chat.status === statusFilter;
      const matchesProcedence = !procedenceFilter || chat.leadData?.procedence === procedenceFilter;
      const matchesTag = !tagFilter || (chat.tags && chat.tags.some(tag => 
        tag.toLowerCase().includes(tagFilter.toLowerCase())
      ));
      
      return matchesSearch && matchesStatus && matchesProcedence && matchesTag;
    })
    .sort((a, b) => {
      switch (sortBy) {
        case 'name':
