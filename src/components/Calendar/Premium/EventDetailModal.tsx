...
import { 
  X, Calendar, Clock, MapPin, Users, Link, FileText, 
  Video, Phone, Mail, Edit3, Trash2, Copy, 
  ExternalLink, Bell, MoreHorizontal
} from 'lucide-react';
...
export const EventDetailModal: React.FC<EventDetailModalProps> = ({
  ...
  const getEventTypeIcon = () => {
    if (event.meeting_link) return <Video className="h-5 w-5" />;
            ...
            <div>
              {event.is_all_day ? (
                <p className={darkMode ? 'text-gray-200' : 'text-gray-800'}>
                  Todo el día • {format(new Date(event.start_datetime), "EEEE, d 'de' MMMM 'de' yyyy", { locale: es })}
                </p>
              ) : (
                <>
                  <p className={darkMode ? 'text-gray-200' : 'text-gray-800'}>
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    hasta {formatDateTime(new Date(event.end_datetime))}
                  </p>
                </>
              )}
            </div>
                  ...
                  {event.attendees.map((attendee: any, index: number) => (
                    <div key={index} className="flex items-center space-x-3">
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        ...
                        {attendee.status === 'accepted' ? 'Asistirá' :
                         attendee.status === 'declined' ? 'No asistirá' :
                         attendee.status === 'tentative' ? 'Tal vez' : 'Pendiente'}
                      </span>
                    </div>
                  ))}
                </div>
